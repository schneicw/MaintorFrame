...
// CLASSES
var Mentor = function(name, gender, language, transport, transport_check, mon_t, tues_t, weds_t, thurs_t, fri_t){
  this.name = name;
  this.gender = gender;
  this.language = language;
  this.transport = transport;
  this.can_drive = transport_check;
...
var lunches_dimensions = lunch_sheet.getDataRange();

for (var i = 0; i < data_mentor.length; i++){
  for (var j = 0; j < 30; j++){
    if (!(data_mentor[i][j] == undefined)){
      data_mentor[i][j] = data_mentor[i][j].toString().trim()
    }
  }
}
  
for (var i = 0; i < data_mentee.length; i++){
  for (var j = 0; j < 30; j++){
    if (!(data_mentee[i][j] == undefined)){
      data_mentee[i][j] = data_mentee[i][j].toString().trim();
    }
  }
}
  
for (var i = 0; i < school_lunches.length; i++){
  for (var j = 0; j < 20; j++){
    if (!(school_lunches[i][j] == undefined)){
      school_lunches[i][j] = school_lunches[i][j].toString().trim();
    }
  }
}
...
// STUFF THAT CHARLIE ADDED
var mentees = [];
for(var i = 1; i < data_mentee.length; i++){
  mentees.push(data_mentee[i][mentee_name_col]);
}
...
function trim_whitespace(){
  for (var i = 0; i < data_mentor.length; i++){
    for (var j = 0; j < mentor_dimensions.length; j++){
      data_mentor[i][j] = data_mentor[i][j].trim();
    }
  }
  
  for (var i = 0; i < data_mentee.length; i++){
    for (var j = 0; j < mentee_dimensions.length; j++){
      data_mentee[i][j] = data_mentee[i][j].trim();
    }
  }
  
  for (var i = 0; i < school_lunches.length; i++){
    for (var j = 0; j < lunches_dimensions.length; j++){
      school_lunches[i][j] = school_lunches[i][j].trim();
    }
  }
}
...
function check_preferences(){
  for (var i = 1; i < data_mentor.length; i++){
    ...
    var has_transport = "No";
    transportation_list = transportation_list.split(",");
    if (transportation_list.indexOf("I have a car and am willing to drive a carpool to my school") >= 0){
        transport = "Has car and is willing to drive a carpool";
        has_transport = "Yes";
        
      }
    else if (transportation_list.indexOf("I have a car and am willing to drive myself to my school") >= 0){
        transport = "Has car";
        has_transport = "Yes";
      }
    else if (transportation_list.indexOf("I can ride in a carpool to get to my school") >= 0){
        transport = "Needs carpool unless biking or walking";
        has_transport = "Yes";
          if (transportation_list.indexOf("I have a bike and am willing to ride my bike to a school nearby")>=0){
            transport += ", has a bike or can walk";
          }
          else{
            transport += ", doesn't have a bike but can walk";
          }
    }
    else{
      if (transportation_list.indexOf("I have a bike and am willing to ride my bike to a school nearby")>=0){
    ...
    for (var day = 0; day < 5; day++){
       ...
       if (!(available == "")){
         ...
         var times_available = (connecting_times(convert_to_military(available)));
         if (day == 0){
    ...
    var mentor = new Mentor(mentor_name, gender, language, transport, has_transport, temp_dict["Monday"], temp_dict["Tuesday"], temp_dict["Wednesday"], temp_dict["Thursday"], temp_dict["Friday"]);
...
// THIS FUNCTION REQUIRES THAT 1:00PM, 2:00PM, etc. comes after the earlier times in each list

// https://stackoverflow.com/questions/29206453/best-way-to-convert-military-time-to-standard-time-in-javascript

function convert_to_military(input_list){
  
  for (var i = 0; i < (input_list.length); i++){
    var current_time = input_list[i];
    
    var left_time = current_time.slice(0, current_time.indexOf("-"));
    var left_converted_time = left_time;
    var left_time = left_time.split(":");
    var left_hours = Number(left_time[0]);
    
    if (left_hours > 0 && left_hours <= 6){
      left_converted_time = ((12 + left_hours).toString() + ":" + left_time[1]);
    }
    
    var right_time = current_time.slice((current_time.indexOf("-")+1));
    var right_converted_time = right_time;
    var right_time = right_time.split(":");
    var right_hours = Number(right_time[0]);
    
    if (right_hours > 0 && right_hours <= 6){
      right_converted_time = ((12 + right_hours).toString() + ":" + right_time[1]);
    }
   
    input_list[i] = left_converted_time + "-" + right_converted_time;
  }
  return input_list;
}
